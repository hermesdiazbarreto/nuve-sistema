...
def reset_sequences(request):
    ...
    Uso: POST /api/reset-sequences/

    IMPORTANTE: Este endpoint debería ser protegido o eliminado en producción.
    """
    try:
        tablas = [
            'alm_categoria',
        ...
        return Response({
            'message': 'Secuencias reseteadas correctamente',
            'resultados': resultados
        })

    except Exception as e:
        return Response(
            {'error': f'Error al resetear secuencias: {str(e)}'},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
